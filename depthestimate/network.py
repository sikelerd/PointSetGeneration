...
def build_graph(resourceid, img_height, img_width, pointcloudsize, outputpoints, learning_rate):
    with tf.device('/gpu:%d' % resourceid):
        tflearn.init_graph(seed=1029, num_cores=2, gpu_memory_fraction=0.9, soft_placement=True)
        img_inp = tf.placeholder(tf.float32, shape=(None, img_height, img_width, 4), name='img_inp')
