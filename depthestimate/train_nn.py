...
def build_graph(resourceid):
    with tf.device('/gpu:%d' % resourceid):
        tflearn.init_graph(seed=1029, num_cores=2, gpu_memory_fraction=0.9, soft_placement=True, distribution='uniform')
        img_inp = tf.placeholder(tf.float32, shape=(None, HEIGHT, WIDTH, 4), name='img_inp')
        batch = tf.shape(img_inp)[0]
        print(tf.shape(img_inp))
...
if __name__ == '__main__':
    ...
    print
    "datadir=%s dumpdir=%s num=%d cmd=%s started" % (datadir, dumpdir, valnum, cmd)
